...
- Single-column `UPDATE` support with row rewrites and table-level index rebuilds
...
UPDATE users SET name = 'Ava' WHERE id = 1;
...
- Single-column `SET column = literal` updates only
- `LIMIT` on `SELECT`
- One table per query
- No joins, `ORDER BY`, or `GROUP BY`
